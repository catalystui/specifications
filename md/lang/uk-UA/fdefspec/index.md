...
> **Специфікація фундаментальних визначень**<br/>
> Ревізія 1<br/>
> 23 березня 2026 р.<br/>
> <br/>
> Copyright © 2026 CatalystUI LLC. <br/>
> Усі права захищено.<br/>
> <br/>
> Подані тут визначення й поняття описують фундаментальні математичні конструкції та можуть бути вільно переказані.

## Вступ

**Foundational Definitions Specification (FDEFSPEC)** встановлює основні поняття й термінологію, що лежать в основі екосистеми CatalystUI. Її мета — забезпечити єдине розуміння цих понять у документації, специфікаціях, реалізаціях і верифікаційних розглядах, забезпечуючи послідовність, ясність і узгодженість.

Визначаючи ключові терміни й зв'язки в точній та стабільній формі, FDEFSPEC надає спільну точку відліку для специфікацій вищого рівня. Це дозволяє розробникам, рецензентам і реалізаторам міркувати з однієї основи, коли вони визначають, чи можна мову, сервіс, framework або систему вважати within spec.

> [!IMPORTANT]
>
> Ми виражаємо визначення за допомогою похідної форми нотації [теорії множин](https://en.wikipedia.org/wiki/Set_theory). Такий підхід забезпечує точні, недвозначні визначення, зберігаючи ясність і стислість. Ми структуруємо ці визначення так, щоб підтримувати зручне посилання, чітке тлумачення та послідовну концептуальну ієрархію.
...
Біт — це будь-яке $b \in \{0,1\}$.

### Nibble

Нібл — це будь-який $(b_0,\dots,b_3)$ такий, що для всіх $i \in \{0,\dots,3\}$, $b_i \in \mathrm{Bit}$.
...
Байт — це будь-який $(b_0,\dots,b_7)$ такий, що для всіх $i \in \{0,\dots,7\}$, $b_i \in \mathrm{Bit}$.
...
Наведене нижче визначення відповідає стандарту IEEE 754 для представлення чисел з рухомою комою одинарної точності. Воно визначає, як 32-бітова двійкова послідовність інтерпретується як число з рухомою комою, включно зі спеціальними випадками нуля, нескінченності та NaN (Not a Number).
...
Наведене нижче визначення відповідає стандарту IEEE 754 для представлення чисел з рухомою комою подвійної точності. Воно визначає, як 64-бітова двійкова послідовність інтерпретується як число з рухомою комою, включно зі спеціальними випадками нуля, нескінченності та NaN (Not a Number).
...
Кодування тексту визначається як така множина:
...
Unicode codepoint — це будь-яке $e \in \mathbb{Z}$ таке, що $0 \leq e \leq 0x10FFFF$.
...
Послідовність називається null-terminated.
...
| Код | Рівень | Категорія | Опис | Деталь |
| --- | --- | --- | --- | --- |
| `0x00` | SUCCESS | NONE | Операцію успішно завершено. | |
| `0x01` | SUCCESS | NOOP | Операцію успішно завершено без виконання будь-якої дії. | |

##### Warning

| Код | Рівень | Категорія | Опис | Деталь |
| --- | --- | --- | --- | --- |
| `0x40` | WARNING | NONE | Операцію завершено, але результат може бути неочікуваним або небажаним. | |
| `0x41` | WARNING | PARTIAL | Операцію завершено, але лише частково. Вивід може бути неповним. | |
...
| Код | Рівень | Категорія | Опис | Деталь |
| --- | --- | --- | --- | --- |
| `0x80` | ERROR | NONE | Операція не вдалася через помилку, яку можна відновити. | |
...
| Код | Рівень | Категорія | Опис | Деталь |
| --- | --- | --- | --- | --- |
| `0xC0` | FATAL | NONE | Операція не вдалася через невідновну помилку. | |

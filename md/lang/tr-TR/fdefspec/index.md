...
> **Temel Tanımlar Spesifikasyonu**<br/>
> Revizyon 1<br/>
> 23 Mart 2026<br/>
> <br/>
...
> <br/>
> Burada sunulan tanımlar ve kavramlar temel matematiksel yapıları açıklar ve serbestçe yeniden ifade edilebilir.

## Giriş

**Foundational Definitions Specification (FDEFSPEC)**, CatalystUI ekosisteminin temelini oluşturan ana kavramları ve terminolojiyi belirler. Amacı, belgeler, spesifikasyonlar, uygulamalar ve doğrulama incelemeleri boyunca bu kavramlara ilişkin birleşik bir anlayış sağlayarak tutarlılık, açıklık ve uyum oluşturmaktır.

FDEFSPEC, önemli terimleri ve ilişkileri kesin ve kararlı bir biçimde tanımlayarak daha üst düzey spesifikasyonlar için ortak bir başvuru noktası sağlar. Bu, geliştiricilerin, inceleyicilerin ve uygulayıcıların bir dilin, hizmetin, framework'ün veya sistemin within spec kabul edilip edilemeyeceğini belirlerken aynı temelden akıl yürütmesine olanak tanır.

> [!IMPORTANT]
>
> Tanımları [küme teorisi](https://en.wikipedia.org/wiki/Set_theory) gösteriminin türetilmiş bir biçimiyle ifade ederiz. Bu yaklaşım, açıklığı ve kısalığı korurken kesin ve belirsiz olmayan tanımlar sağlar. Bu tanımları kolay başvuru, açık yorumlama ve tutarlı bir kavramsal hiyerarşiyi destekleyecek şekilde yapılandırırız.
...
Bit, $b \in \{0,1\}$ olan herhangi bir değerdir.

### Nibble

Nibble, tüm $i \in \{0,\dots,3\}$ için $b_i \in \mathrm{Bit}$ olacak şekilde herhangi bir $(b_0,\dots,b_3)$ dizisidir.
...
Byte, tüm $i \in \{0,\dots,7\}$ için $b_i \in \mathrm{Bit}$ olacak şekilde herhangi bir $(b_0,\dots,b_7)$ dizisidir.
...
Aşağıdaki tanım, tek duyarlıklı kayan nokta temsili için IEEE 754 standardına uygundur. Sıfır, sonsuzluk ve NaN (Not a Number) özel durumları dahil olmak üzere 32 bitlik bir ikili dizinin kayan noktalı sayı olarak nasıl yorumlandığını tanımlar.
...
Aşağıdaki tanım, çift duyarlıklı kayan nokta temsili için IEEE 754 standardına uygundur. Sıfır, sonsuzluk ve NaN (Not a Number) özel durumları dahil olmak üzere 64 bitlik bir ikili dizinin kayan noktalı sayı olarak nasıl yorumlandığını tanımlar.
...
Boolean, $b \in \{0,1\}$ olan herhangi bir değerdir.
...
Bir metin kodlaması aşağıdaki küme olarak tanımlanır:
...
Unicode codepoint, $0 \leq e \leq 0x10FFFF$ olacak şekilde herhangi bir $e \in \mathbb{Z}$ değeridir.
...
Dizinin null-terminated olduğu söylenir.
...
| Kod | Seviye | Kategori | Açıklama | Ayrıntı |
| --- | --- | --- | --- | --- |
| `0x00` | SUCCESS | NONE | İşlem başarıyla tamamlandı. | |
| `0x01` | SUCCESS | NOOP | İşlem herhangi bir eylem gerçekleştirmeden başarıyla tamamlandı. | |

##### Warning

| Kod | Seviye | Kategori | Açıklama | Ayrıntı |
| --- | --- | --- | --- | --- |
| `0x40` | WARNING | NONE | İşlem tamamlandı, ancak sonuç beklenmeyen veya istenmeyen olabilir. | |
| `0x41` | WARNING | PARTIAL | İşlem tamamlandı, ancak yalnızca kısmen. Çıktı eksik olabilir. | |
...
| Kod | Seviye | Kategori | Açıklama | Ayrıntı |
| --- | --- | --- | --- | --- |
| `0x80` | ERROR | NONE | İşlem kurtarılabilir bir hata nedeniyle başarısız oldu. | |
...
| Kod | Seviye | Kategori | Açıklama | Ayrıntı |
| --- | --- | --- | --- | --- |
| `0xC0` | FATAL | NONE | İşlem kurtarılamaz bir hata nedeniyle başarısız oldu. | |
